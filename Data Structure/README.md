...
넣고자 하는 값이 현재 노드의 값보다 크면 오른쪽 서브트리로 이동하고

넣고자 하는 값이 현재 노드의 값보다 작으면 왼쪽 서브트리로 이동한다.

이동할 서브트리가 비어있으면 노드를 만들어 넣고자 하는 값을 저장한다.
...
2. 삭제하려는 노드의 서브트리가 1개인 경우

   노드를 삭제하고 부모노드와 서브트리를 연결하면 된다.

3. 삭제하려는 노드의 서브트리가 2개인 경우

   방법 1 : 삭제하려는 노드의 왼쪽 서브트리 중 가장 큰 노드를 삭제하려는 노드의 자리에 올린다.

   방법 2 : 삭제하려는 노드의 오른쪽 서브트리 중 가장 작은 노드를 삭제하려는 노드의 자리에 올린다.
