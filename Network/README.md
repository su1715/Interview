...
- UDP 역다중화 : 소켓을 식별하기 위해 목적지의 IP와 포트번호를 사용한다.

- TCP 역다중화 : 소켓을 식별하기 위해 출발지 IP, 포트번호, 도착지 IP, 포트번호를 모두 사용한다.
...
URL 을 통해 자원을 명시하고, Method 를 통해 해당 자원애 대한 CRUD 를 지원하는 것

- REST API : REST 를 지원하는 API
